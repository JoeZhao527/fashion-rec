...
def aggregate_recommendations(user_profiles, indices, recent_transactions, k=12):
    ...
    # Recommend most popular items within user's cluster
...
def create_faiss_index(user_profiles, use_gpu=False, k=200):
    """
    Get most similar k users for each user
    """
...
class Item2VecModel:
    def __init__(self, train: pd.DataFrame, articles: pd.DataFrame, top_N: int, model_cfg: dict, *args, **kwargs):
        ...
        # Item2Vec model training. User's purchase history as sentence, each item as a word (token)
        model = Word2Vec(sentences=training_data, **model_cfg)

        # Store embedding for each item, which can reflect items' distances based on whether they are bought together
        item_vectors = {item: model.wv[item] for item in model.wv.index_to_key}
        vector_size = model.vector_size

        # Build user profile based on items feature vector
    ...
    def similarity_recall(self):
        """
        Recommendation based on similarity between user profile and items
        """
    ...
    def cluster_recall(self):
        """
        Recommendation based on clustering users by their profile, and recommend
        popular items in the cluster that user belongs to.
        """
